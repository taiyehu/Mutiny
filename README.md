...
- 房主逐个批准访客，并可将其设为红方玩家或只读观众；
- 使用独立的红方两点校准阶段；校准完成后蓝方房主固定先手，蓝红双方点击“结束回合”交接；
- 蓝红双方正式回合默认各有 90 秒倒计时，归零后由服务端自动交棒；
...
项目把两条 WebRTC 通道与服务端回合状态结合起来：

1. **MediaStream**：房主调用 `getDisplayMedia()` 获取用户选择的画面和音频，然后将媒体轨道加入 `RTCPeerConnection`。
2. **RTCDataChannel**：当前持有控制令牌的红方把归一化后的鼠标坐标、点击和按键事件发送给房主；非当前回合的输入会在发送端和房主端各拦截一次。
3. **校准与回合控制令牌**：信令服务是控制权的唯一裁判。准备完成后先把输入临时交给红方做两点校准，校准不计入回合；启用页面控制后蓝方进入第 1 回合，蓝方交棒给红方、红方交还蓝方。每回合默认 90 秒，归零自动轮换；房主强制收回、成员断线、权限降级以及失去心跳都会安全回到准备阶段或蓝方。
...
1. 在两个浏览器窗口中分别打开 `http://localhost:3000/`；
...
如果鼠标仍有明显偏移，先保持“页面控制”关闭并点击“校准坐标”。目标页面会依次显示 `1 / 2` 和 `2 / 2` 两个圆形定位点，请让访客在共享画面中准确点击它们。助手会根据这两个样本计算横纵方向各自的缩放和偏移；显示“校准完成”后，再点击“启用页面控制”。切换目标标签页后需要重新校准。
