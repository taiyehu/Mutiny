...
type ControlState = { ownerPeerId: string; ownerName: string; side: "blue" | "red"; phase: "setup" | "calibration" | "ready" | "playing"; turnNumber: number; turnDeadline: number | null; leaseUntil: number };
...
export default function Home() {
  ...
  const bindChannel = useCallback((peerId: string, channel: RTCDataChannel) => {
    ...
    channel.onmessage = (event) => {
      ...
      if (roleRef.current === "host" && controlRef.current?.ownerPeerId !== peerId) return;
  ...
  const connectSignaling = useCallback(() => new Promise<WebSocket>((resolve, reject) => {
    ...
    socket.onmessage = (event) => {
      ...
      if (message.type === "room-state") { peersRef.current = message.peers; controlRef.current = message.control; setPeers(message.peers); setControl(message.control); }
  ...
  }), [applySignal, createHostPeer, stopRtc]);
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      socket.send(JSON.stringify({ type: "create-room", name: name.trim() || "房主" }));
    } catch (error) {
  ...
  const sendControl = (event: RemoteEvent) => {
    if (peerRole !== "controller" || controlRef.current?.ownerPeerId !== selfPeerIdRef.current) return;
  ...
  const connectCompanion = () => {
    ...
    socket.onopen = () => socket.send(JSON.stringify({ type: "auth", code: companionCode.trim() }));
    socket.onmessage = (event) => {
      const message = JSON.parse(event.data);
      ...
      if (message.type === "calibration-state") {
        ...
        setLastCommand(message.state === "point-1" ? "请让红方点击画面中的定位点 1 / 2" : message.state === "point-2" ? "请让红方点击画面中的定位点 2 / 2" : message.state === "complete" ? "坐标校准完成，蓝方第 1 回合开始" : "坐标校准已停止");
      ...
      if (message.type === "auth-error") { setCompanionState("off"); setNotice("浏览器助手授权码不正确。"); }
    };
    socket.onerror = () => { setCompanionState("off"); setNotice("无法连接浏览器助手，请先运行 npm run companion:arm。"); };
    socket.onclose = () => {
      desktopControlEnabledRef.current = false;
      setDesktopControlEnabled(false);
      setCompanionState("off");
  ...
  const startCalibration = () => {
    setNotice("");
    const calibrationPeer = peersRef.current.find((peer) => peer.approved && peer.role === "controller");
    if (!calibrationPeer) return setNotice("请先批准一名红方玩家，再开始校准。");
    sendSignal({ type: "start-calibration", peerId: calibrationPeer.peerId });
  ...
  useEffect(() => {
    if (control?.phase !== "playing" || !control.turnDeadline) return;
  ...
  }, [control?.phase, control?.turnDeadline]);
  ...
  const canGuestControl = role === "guest" && peerRole === "controller" && hasControl;
  const turnLabel = control?.phase === "playing" ? `第 ${control.turnNumber} 回合 · ${turnSecondsLeft ?? "—"} 秒` : control?.phase === "calibration" ? "校准阶段" : control?.phase === "ready" ? "校准完成" : "准备阶段";
  const turnTitle = control?.phase === "setup" ? "等待红方与校准" : control?.phase === "calibration" ? `红方校准 · ${control.ownerName}` : control?.phase === "ready" ? "等待启用页面控制" : control?.side === "red" ? `红方 · ${control.ownerName}` : "蓝方 · 房主";
  const turnDescription = control?.phase === "setup" ? role === "host" ? "批准红方并连接页面助手，然后开始两点校准。" : "等待房主启动两点校准；校准完成后蓝方先手。" : control?.phase === "calibration" ? hasControl ? "请依次点击共享画面中的两个定位点；这不计入正式回合。" : "正在等待红方完成两点校准，本地游戏输入已锁定。" : control?.phase === "ready" ? role === "host" ? "点击“启用页面控制”后，蓝方第 1 回合与倒计时同时开始。" : "校准已完成，等待房主启用页面控制。" : hasControl ? "轮到你操作；倒计时结束会自动交棒，也可以主动结束回合。" : "当前输入已锁定，请等待对方结束回合或倒计时归零。";

  return <main className="shell">
    <nav className="topbar">
      <div className="brand"><span className="brandMark">M</span><span>Mutiny Relay</span></div>
      <span className="prototypeTag">LOCAL DEV · 本地联机</span>
    </nav>
    ...
    {!role && <>
      <section className="hero compactHero">
        ...
        <h1>本地房间，<br />即开即玩</h1>
        <p>启动本地服务后，用 6 位房间码连接。房主决定谁能操作，谁只能观战。</p>
    ...
    {role && <section className="workspace">
      <div className="workspaceTitle"><div><span className={`rolePill ${role === "host" ? "blueSide" : "redSide"}`}>{role === "host" ? "蓝方房主" : peerRole === "controller" ? "红方玩家" : "观众"}</span><h1>{role === "host" ? `房间 ${roomCode || "······"}` : `加入 ${roomCode || joinCode}`}</h1></div><button className="textButton" onClick={role === "host" ? closeHostedRoom : reset}>{role === "host" ? "关闭房间" : "退出房间"}</button></div>
      ...
      <div className="grid">
        ...
        <aside className="panel roomPanel">
          <div className={`turnCard ${control?.side || "blue"}`}><span>{turnLabel}</span><strong>{turnTitle}</strong><p>{turnDescription}</p>{control?.phase === "playing" && hasControl && (role === "host" ? <button disabled={!remoteControllers.length || !desktopControlEnabled} onClick={() => sendSignal({ type: "pass-control" })}>{!remoteControllers.length ? "等待红方操作者" : !desktopControlEnabled ? "请先启用页面控制" : "结束蓝方回合，交给红方"}</button> : peerRole === "controller" ? <button onClick={() => sendSignal({ type: "pass-control" })}>结束红方回合，交还蓝方</button> : null)}{role === "host" && control?.phase !== "setup" && control?.phase !== "ready" && !hasControl && <button className="reclaimButton" onClick={() => sendSignal({ type: "reclaim-control" })}>{control?.phase === "calibration" ? "取消校准并收回" : "强制收回控制权"}</button>}</div>
          {role === "host" ? <>
            ...
            {pendingPeers.map((peer) => <div className="peer pending" key={peer.peerId}><div><strong>{peer.name}</strong><small>申请：{peer.role === "controller" ? "红方" : "观战"}</small></div><div className="peerActions"><button onClick={() => approve(peer.peerId, "controller")}>设为红方</button><button onClick={() => approve(peer.peerId, "spectator")}>仅观战</button><button className="danger" onClick={() => sendSignal({ type: "reject-peer", peerId: peer.peerId })}>拒绝</button></div></div>)}
            {approvedPeers.map((peer) => <div className="peer" key={peer.peerId}><div><strong>{peer.name}</strong><small>{peer.role === "controller" ? control?.phase === "calibration" && control.ownerPeerId === peer.peerId ? "正在完成校准" : control?.ownerPeerId === peer.peerId ? "红方正在操作" : "红方候选，等待轮换" : "只读观战"}</small></div><button className="roleToggle" onClick={() => setRemoteRole(peer.peerId, peer.role === "controller" ? "spectator" : "controller")}>{peer.role === "controller" ? "改为观战" : "设为红方"}</button></div>)}
            <div className="companionBox"><h2>Chromium 页面控制助手</h2><p>先用远程调试端口启动一个独立 Chromium，再运行 <code>npm run companion:arm</code>。助手只向选定标签页发送事件，不会移动系统鼠标。</p><div><input value={companionCode} onChange={(event) => setCompanionCode(event.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="授权码" maxLength={6} /><button disabled={companionCode.length !== 6 || companionState === "connecting" || companionState === "ready"} onClick={connectCompanion}>{companionState === "ready" ? "已连接" : "连接"}</button></div>{companionState === "ready" && <><div className="targetPicker"><select aria-label="选择要控制的 Chromium 标签页" value={browserTargetId} onChange={(event) => selectBrowserTarget(event.target.value)}><option value="">选择目标标签页</option>{browserTargets.map((target) => <option key={target.id} value={target.id}>{target.title || target.url}</option>)}</select><button aria-label="刷新标签页列表" onClick={() => companionRef.current?.send(JSON.stringify({ type: "refresh-targets" }))}>刷新</button></div><div className="calibrationGate"><button disabled={!browserTargetReady || !remoteControllers.length || desktopControlEnabled || calibrationState === "point-1" || calibrationState === "point-2"} onClick={startCalibration}>{calibrationState === "complete" ? "重新校准坐标" : calibrationState === "point-1" || calibrationState === "point-2" ? "正在等待红方点击" : "开始红方两点校准"}</button><small>{calibrationState === "point-1" ? "请红方点击共享画面中的定位点 1 / 2" : calibrationState === "point-2" ? "请红方点击共享画面中的定位点 2 / 2" : calibrationState === "complete" ? "校准完成；启用页面控制后开始蓝方第 1 回合" : remoteControllers.length ? "校准是独立阶段，完成后蓝方固定先手" : "请先批准一名红方玩家"}</small></div><div className={`controlGate ${desktopControlEnabled ? "armed" : ""}`}><button disabled={!browserTargetReady || calibrationState === "point-1" || calibrationState === "point-2"} onClick={toggleDesktopControl}>{desktopControlEnabled ? "停止页面控制" : "启用页面控制"}</button><small>{desktopControlEnabled ? `事件只会发送到当前锁定的标签页${browserViewport ? `（${browserViewport.width} × ${browserViewport.height} CSS px）` : ""}` : browserTargetReady ? `安全预览模式；目标 viewport ${browserViewport ? `${browserViewport.width} × ${browserViewport.height} CSS px` : "已读取"}` : "请先选择并锁定一个目标标签页"}</small></div></>}</div>
          </> : <>
            <div className="roomCode"><span>当前房间</span><strong>{roomCode || joinCode}</strong><small>{peerRole === "controller" ? control?.phase === "calibration" && hasControl ? "红方校准阶段" : hasControl ? "轮到红方，你可以操作" : "等待蓝方交出控制权" : "你处于只读观战模式"}</small></div>
            <div className={`permissionCard ${peerRole}`}><b>{peerRole === "controller" ? control?.phase === "calibration" && hasControl ? "CAL" : hasControl ? "PLAY" : "WAIT" : "VIEW"}</b><strong>{peerRole === "controller" ? control?.phase === "calibration" && hasControl ? "两点校准" : hasControl ? "红方回合" : "等待蓝方回合" : "只读观战"}</strong><p>{peerRole === "controller" ? control?.phase === "calibration" && hasControl ? "依次点击两个定位点；正式回合与倒计时尚未开始。" : hasControl ? "移动和点击视频画面，聚焦后可使用方向键；完成后点击结束回合。" : "视频仍可观看，但鼠标和键盘事件不会发送。" : "房主可以把你设为红方玩家。"}</p></div>
          </>}
