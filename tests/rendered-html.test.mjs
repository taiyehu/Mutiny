...
test("本地联机组件与配置齐全", async () => {
  ...
  assert.match(page, /jitterBufferTarget\s*=\s*kind === "video" \? 0 : 80/);
  ...
  assert.match(page, /pc\.addTrack\(track, new MediaStream\(\[track\]\)\)/);
  assert.match(page, /const isolatedStream = new MediaStream\(\[event\.track\]\)/);
  assert.match(page, /<audio ref=\{remoteAudioRef\}/);
  assert.match(page, /audioJitterBuffer/);
  assert.match(page, /estimatedPlayoutTimestamp/);
  assert.match(page, /启用声音/);
  assert.doesNotMatch(page, /srcObject = event\.streams\[0\]/);
  ...
  assert.match(styles, /audioDelayMetric/);
  ...
  assert.match(styles, /videoViewport:fullscreen \.mobileJoystick/);
